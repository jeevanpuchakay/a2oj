...
int main(int argc, char **argv)
{
    ...
    freopen("outputWithSerialProgram.txt", "w", stdout);
    int rank, size;
    MPI_Init(&argc, &argv);
    ...
    if (rank == 0)
    {
        ...
        cout << "      Node1Id     "
             << "      Node2Id       " << endl;
        for (int eachDegree = 0; eachDegree < 90; eachDegree++)
            checkForNodesWithIn5M(eachDegree);
    }
    MPI_Finalize();

    auto stop = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(stop - start);
    cout << "Time taken by Serial program: "
...
//mpic++ SerialSolution.cpp -o serialSolution
//mpirun -np 1 ./serialSolution